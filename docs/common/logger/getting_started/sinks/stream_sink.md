...
It does not replace the context event policy. The event policy decides which events the context passes to the sink at all. The handler level is applied later, inside the stream sink, during delivery through standard `logging`.
