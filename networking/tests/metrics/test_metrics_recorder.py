# tests/metrics/test_metrics_recorder.py
...
from mvx.networking.metrics.asyncio_metrics_recorder import metrics_recorder as recorder_pack
from mvx.networking.metrics.asyncio_metrics_recorder.common import (
    AsyncioMetricsRecorderState,
)
from mvx.networking.metrics.asyncio_metrics_recorder.errors import (
...
async def test_d04_on_stopped_ordinary_exception_becomes_stopped_hook_failed() -> None:
    ...
    assert outcome.success is False
    assert isinstance(outcome.error, AsyncioMetricsRecorderStoppedHookFailedError)
    assert recorder.get_status() is AsyncioMetricsRecorderState.FAILURE
    assert recorder._last_error is outcome.error
...
@pytest.mark.asyncio
async def test_m01_dispatch_error_is_logged_when_log_context_is_configured(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(recorder_pack, "LogContext", RecordingLogContext)

    log_context = RecordingLogContext()
    recorder = RecordingMetricsRecorder(
        entity_id="recorder-1",
        log_context=cast(Any, log_context),
    )
    recorder.register_metric(CountingMetric(fail_on_handle=True))
    ...
    await wait_until(
        lambda: any(
            item["event"] == "metrics_recorder.dispatch_error" for item in log_context.error_events
        )
    )

    assert log_context.error_events == [
        {
            "event": "metrics_recorder.dispatch_error",
            "payload": {
                "error_type": "RuntimeError",
                "message": "metric handle failed",
            },
            "entity_id": "recorder-1",
            "skip_payload_normalization": True,
        }
    ]
    assert len(log_context.payload_errors) == 1
    assert isinstance(log_context.payload_errors[0], RuntimeError)


@pytest.mark.asyncio
async def test_m02_cleanup_failure_is_logged_when_log_context_is_configured(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(recorder_pack, "LogContext", RecordingLogContext)
    ...
    await wait_until(
        lambda: any(
            item["event"] == "metrics_recorder.cleanup.failed" for item in log_context.error_events
        )
    )
    ...
    assert {
        ...
        "entity_id": "recorder-1",
        "skip_payload_normalization": True,
    } in log_context.error_events


@pytest.mark.asyncio
async def test_m03_cleanup_task_creation_error_is_logged_when_log_context_is_configured(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(recorder_pack, "LogContext", RecordingLogContext)
    ...
    original_create_task = recorder._loop.create_task

    def create_task_or_fail(
        coro: Any,
        *,
        name: str | None = None,
        context: Any = None,
    ) -> asyncio.Task[Any]:
        if name == "test.metrics.cleanup":
            coro.close()
    ...
    await wait_until(
        lambda: any(
            item["event"] == "metrics_recorder.cleanup.task_creation_error"
            for item in log_context.error_events
        )
    )
    ...
    assert log_context.error_events == [
        {
            ...
            "payload": {
                "error_type": "RuntimeError",
                "message": "cleanup task creation failed",
            },
            "entity_id": "recorder-1",
            "skip_payload_normalization": True,
        }
...
@pytest.mark.asyncio
async def test_m04_cleanup_failure_without_log_context_is_suppressed() -> None:
    recorder = RecordingMetricsRecorder(entity_id="recorder-1")
    recorder.stopped_exception = RuntimeError("cleanup failed")
...
@pytest.mark.asyncio
async def test_m05_metric_changed_error_is_logged_when_log_context_is_configured(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(recorder_pack, "LogContext", RecordingLogContext)
