# src/mvx/networking/metrics/metrics_runtime/metrics_runtime.py
    ...
    def _build_shutdown_error_payload(
        ...
        return {
            ...
            "recorder_errors": {
                ...
                for recorder_id, exc in errors.items()
            },
        }
    ...
    async def _shutdown_loop_core(self) -> None:
        ...
        if shutdown_errors:
            ...
            raise MetricsRuntimeShutdownError(
                cause=first_error,
                details=self._build_shutdown_error_payload(shutdown_errors),
            )

    # ---- Recorders management ------------------------------------------------------------
