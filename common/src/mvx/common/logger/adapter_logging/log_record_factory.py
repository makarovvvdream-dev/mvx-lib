...
The standard ``LogRecord.message`` value is built from MVX event identity:
    ...
    event_namespace[.entity_id].event_name[ [event_outcome]]
...
``event_namespace``, ``entity_id``, and ``event_outcome`` are included in the
...
these MVX-specific fields:
    ...
    event_outcome
        Event outcome from ``LogEvent.event_outcome`` or ``"<not defined>"``.
...
def make_log_record_from_event(logger_name: str, event: LogEvent) -> logging.LogRecord:
    ...
    Returns:
        ...
        fields such as ``event_namespace``, ``event_name``, ``event_outcome``,
    ...
    if event.event_outcome:
        msg += f" [{event.event_outcome}]"

    event_namespace = event.meta.event_namespace or "<not defined>"
    event_outcome = event.event_outcome or "<not defined>"
    ...
    record.event_outcome = event_outcome
